...
$(function() {
    ...
    $("#pc_success_logout").click(function(e){
        e.preventDefault();
        var uname = $("#username"),
            uip = $("#user_ip");
        //logout  注销
        ...
        $.getJSON(location.protocol + "//" + location.hostname + "/cgi-bin/srun_portal", pData, function(data) {
            if (data.error == "ok") {
                alert("下线成功（logout success）");
                window.close();
                //location.href = location.protocol + "//" + location.hostname+'?is_logout=1';
                return false;
            }else {
                alert(data.error_msg);
            }
    ...
    $("#pc_logout").click(function(e){
        ...
        $.getJSON(location.protocol + "//" + location.hostname + "/cgi-bin/srun_portal", pData, function(data) {
            if (data.error == "ok") {
                alert("下线成功（logout success）");
                location.href = location.protocol + "//" + location.hostname+'/srun_portal_pc.php?ac_id='+GetQueryString('ac_id');
                return false;
            }else {
                alert(data.error_msg);
            }
    ...
    $("#phone_logout").click(function(e){
        ...
        $.getJSON(location.protocol + "//" + location.hostname + "/cgi-bin/srun_portal", pData, function(data) {
            if (data.error == "ok") {
                alert("下线成功（logout success）");
                location.href = location.protocol + "//" + location.hostname+'/srun_portal_phone.php?ac_id='+GetQueryString('ac_id');
                return false;
            }else {
                alert(data.error_msg);
            }
    ...
    $("#phone-form2").submit(function(e){
        ...
        $.getJSON(location.protocol + "//" + location.hostname + "/cgi-bin/srun_portal", qData, function(data) {
            if (data.error == "ok") {
                ...
                location.href=location.protocol + "//" + location.hostname + "/srun_portal_phone.php?ac_id=" + acid.val() + "&username=" + data.username + "&ip=" + data.client_ip + "&access_token=" + data.access_token;
                return false;
            }
            for (var i in PortalError) {
                if (data.error_msg.indexOf(i) >= 0) {
